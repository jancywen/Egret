...
class CameraController: UIViewController {
    ...
    public var delegate : CameraControllerProtocol!
    
    

    convenience init(type: CameraType = .ID, idType: IDCardType = .front, blType: BLType = .horizontal) {
        self.init()
    ...
    lazy var floatingView: CameraFloatingView = {
        let v = CameraFloatingView(type: self.type, idType: self.idType, blType: self.blType)
        v.frame = view.bounds
        return v
    }()
    ...
    @objc private func shutterCamera(btn : UIButton) {
        ...
        if let videoConnection = self.imageOutput.connection(with: AVMediaType.video) {
            self.imageOutput.captureStillImageAsynchronously(from: videoConnection) {[weak self] (imageDataSampleBuffer, error) in
                ...
                guard let clipImage = img.dx_clipImageInRect(rect: self.floatingView.WindowLayer.frame) else{ return }
